...
def draw_bbox(image, bbox, color=(0, 255, 0)):
    h, w = image.shape[:2]
    class_id, x_c, y_c, bw, bh = map(float, bbox.strip().split())
    x1 = int((x_c - bw / 2) * w)
    y1 = int((y_c - bh / 2) * h)
    x2 = int((x_c + bw / 2) * w)
    y2 = int((y_c + bh / 2) * h)
...
for label_file in label_files[52:]:
    ...
    # Palmera y ambiente pueden no estar siempre
    palmera_bboxes = []
    ambiente_bboxes = []
    ...
    while index < len(bboxes):
        ...
        # Si hiciste clic izquierdo → actualizar posición y mostrar bbox corregido
        if click_done:
            h, w = image.shape[:2]
            cx, cy = clicked_point
            ...
            # Guardar la corrección en memoria, pero no la escribimos aún
        ...
        elif key == 13:  # ENTER → siguiente
            index += 1

        elif key == ord('q'):  # Q → eliminar bbox y json entry
            ...
            # No se incrementa el índice: muestra el siguiente bbox en la misma posición
